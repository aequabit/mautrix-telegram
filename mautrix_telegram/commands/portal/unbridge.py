...
def _get_portal_murder_function(
    action: str, room_id: str, function: Callable, command: str, completed_message: str, is_forum: bool = False
) -> dict:
...
async def delete_portal(evt: CommandEvent) -> EventID | None:
    portal, forum_mapping = await _get_portal_and_check_permission(evt)
    if not portal:
        return None

    # TODO: Support deletion of forum bridge rooms
    if forum_mapping:
        return None
...
async def unbridge(evt: CommandEvent) -> EventID | None:
    ...
    evt.sender.command_status = _get_portal_murder_function(
        "Room unbridging",
        forum_mapping.mxid if forum_mapping else portal.mxid,
        forum_mapping.unbridge if forum_mapping else portal.unbridge,
        "unbridge",
        "Room successfully unbridged."
    )
    if forum_mapping:
        return await evt.reply(
            f'Please confirm unbridging forum "{forum_mapping.title}" in chat "{portal.title}" '
            f"from room [{forum_mapping.mxid}](https://matrix.to/#/{forum_mapping.mxid}) "
